 ...
 * CORREÇÃO v10.3: suporte a nomes alternativos de variáveis de ambiente:
 *   - SUPABASE_SERVICE_ROLE_KEY (nome padrão Supabase/Vercel) ou SUPABASE_SERVICE_KEY (legado)
 *   - APP_SYNC_KEY (nome na Vercel) ou SYNC_KEY (legado)
 */
...
function getConfig() {
  ...
  if (!url || !key) {
    throw new Error(
      '[supabase] Configure as variáveis de ambiente na Vercel: ' +
      'SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY (ou SUPABASE_SERVICE_KEY).'
    );
  }
  return { url, key };
}
